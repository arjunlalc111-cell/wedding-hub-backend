// env.js - Frontend environment / runtime config (for plain JS/vanilla environments)
...
// Your live backend base URL (for API calls)
// Example: 'https://weddinghub-backend.yourdomain.com/api'
window.BASE_URL = window.BASE_URL || 'https://REPLACE_WITH_YOUR_BACKEND_URL';

// Frontend-wide configuration object
window.FRONTEND_CONFIG = window.FRONTEND_CONFIG || {
  ...
  // WhatsApp number in international format (no +), eg '917727962793'
...
function ensureWhatsAppFloat() {
  try {
    ...
    const waNumber = (cfg.WHATSAPP_NUMBER ||
      window.APP_CONFIG.whatsappNumber || '').toString().trim();
    const a = document.getElementById('whatsappFloat');
    if (!a) return;

    if (waNumber) {
      // ensure international format (no +) for wa.me link
    ...
    if (!img) {
      ...
      a.innerHTML =
        '<svg role="img" aria-label="WhatsApp" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5A9 9 0 1 1 11.5 3"></path><path d="M21 11.5v3.5a2 2 0 0 1-2 2h-1.5"></path><path d="M8.5 10.5c.5 1 1.5 2.2 3 3.2 1.2.8 1.8.9 2.4 1"></path></svg>';
    } else {
